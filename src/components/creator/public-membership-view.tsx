              ...
              >
                ...
                <div className="mt-8 pt-4 border-t border-slate-100">
                  {(() => {
                    ...
                    const currentRank = Number(patron?.tierRank || 0);
                    const isCurrent = isPaidMember && Number(tier.rank) === currentRank;
                    const isLowerOrCurrent =
                      isPaidMember && Number(tier.rank) <= currentRank;
                    ...
                    if (isLowerOrCurrent) {
                      return (
                        <button
                          type="button"
                          disabled
                          className="w-full py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 border border-slate-200 bg-slate-100 text-slate-500 cursor-not-allowed"
                        >
                          <Check className="h-3.5 w-3.5" />
                          <span>{isCurrent ? "Current plan" : "Included in your plan"}</span>
                        </button>
                      );
                    }

                    const isUpgrade = isPaidMember && Number(tier.rank) > currentRank;
                    const href = isFree
