...
export function ClientAdminLoginView({
  slug,
  displayName,
}: {
  slug: string;
  ...
  const { signIn, signOut } = useClientAdminAuthCommands();
  ...
  return (
    <div className="mx-auto flex min-h-[75vh] max-w-md items-center p-6">
      <Card className="w-full">
        <CardHeader>
          <CardTitle>{displayName} admin</CardTitle>
        </CardHeader>
        <CardContent>
          <form
            className="space-y-4"
